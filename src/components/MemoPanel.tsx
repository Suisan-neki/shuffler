...
export default function MemoPanel() {
  ...
  if (isHomeMode && homeImage) {
    return (
      <>
        <div className="fixed inset-0 bg-black/50 z-50" onClick={closeMemoPanel} />
        <div className="fixed top-0 right-0 h-full w-80 bg-gray-900 z-60 flex flex-col shadow-2xl" style={{ zIndex: 60 }}>
          <div className="flex items-center justify-between px-4 pb-2 border-b border-gray-800" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 16px)' }}>
            <p className="text-sm text-gray-400 truncate max-w-[80%]">{homeImage.name}</p>
